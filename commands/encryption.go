...
func GetCryptoCmd() (cli.Command, error) {
	return NewCommand(CommandArgs{
		...
		SynopsisText:  "crypto (<action>)",
		HelpText:      "Encryption-as-a-Service",
		MinNumberArgs: 0,
	})
}

func GetDataKeyCreateCmd() (cli.Command, error) {
	return NewCommand(CommandArgs{
		Path:         []string{cst.NounEncryption, cst.NounDataKey + "/" + cst.Create},
		RunFunc:      encryption{requests.NewHttpClient(), nil}.handleCreateDataKey,
		SynopsisText: "Create a new data key for encryption/decryption",
		HelpText: fmt.Sprintf(`
Usage:
   • %[1]s %[2]s %[3]s --%[4]s mykeys/key1
		`, cst.NounEncryption, cst.NounDataKey, cst.Create, cst.Path),
		FlagsPredictor: cli.PredictorWrappers{
			preds.LongFlag(cst.Path): cli.PredictorWrapper{preds.NewSecretPathPredictorDefault(), preds.NewFlagValue(preds.Params{Name: cst.Path, Shorthand: "r", Usage: fmt.Sprintf("Target %s to a %s (required)", cst.Path, cst.NounDataKey)}), false},
		},
		MinNumberArgs: 1,
	})
}
...
Usage:
   ...
   • %[1]s %[2]s --%[3]s "mykeys/key1 --%[4]s @cipher.enc --%[5]s 0 --%[6]s 3"
		...
		FlagsPredictor: cli.PredictorWrappers{
			...
			preds.LongFlag(cst.VersionStart): cli.PredictorWrapper{complete.PredictAnything, preds.NewFlagValue(preds.Params{Name: cst.VersionStart, Shorthand: "", Usage: "Starting version of the data key (required)"}), false},
			preds.LongFlag(cst.VersionEnd):   cli.PredictorWrapper{complete.PredictAnything, preds.NewFlagValue(preds.Params{Name: cst.VersionEnd, Shorthand: "", Usage: "Ending version of the data key"}), false},
...
func GetDataKeyReadMetadataCmd() (cli.Command, error) {
	return NewCommand(CommandArgs{
		Path:         []string{cst.NounEncryption, cst.NounDataKey + "/" + cst.Read},
		RunFunc:      encryption{requests.NewHttpClient(), nil}.handleReadDataKey,
		SynopsisText: "Read metadata of an existing data key",
		HelpText: fmt.Sprintf(`
Usage:
   • %[1]s %[2]s %[3]s --%[4]s mykeys/key1
		`, cst.NounEncryption, cst.NounDataKey, cst.Read, cst.Path),
		FlagsPredictor: cli.PredictorWrappers{
			preds.LongFlag(cst.Path): cli.PredictorWrapper{preds.NewSecretPathPredictorDefault(), preds.NewFlagValue(preds.Params{Name: cst.Path, Shorthand: "r", Usage: fmt.Sprintf("Target %s to a %s (required)", cst.Path, cst.NounDataKey)}), false},
		},
		MinNumberArgs: 1,
...
func GetDataKeyDeleteCmd() (cli.Command, error) {
	return NewCommand(CommandArgs{
		Path:         []string{cst.NounEncryption, cst.NounDataKey + "/" + cst.Delete},
		RunFunc:      encryption{requests.NewHttpClient(), nil}.handleDeleteDataKey,
		SynopsisText: "Delete an existing data key",
		HelpText: fmt.Sprintf(`
Usage:
   • %[1]s %[2]s %[3]s --%[4]s mykeys/key1
		`, cst.NounEncryption, cst.NounDataKey, cst.Delete, cst.Path),
		FlagsPredictor: cli.PredictorWrappers{
			preds.LongFlag(cst.Path):  cli.PredictorWrapper{preds.NewSecretPathPredictorDefault(), preds.NewFlagValue(preds.Params{Name: cst.Path, Shorthand: "r", Usage: fmt.Sprintf("Target %s to a %s (required)", cst.Path, cst.NounDataKey)}), false},
			preds.LongFlag(cst.Force): cli.PredictorWrapper{complete.PredictAnything, preds.NewFlagValue(preds.Params{Name: cst.Force, Shorthand: "", Usage: fmt.Sprintf("Immediately delete %s and all its versions", cst.NounDataKey), Global: false, ValueType: "bool"}), false},
		},
		MinNumberArgs: 1,
	})
}

func GetDataKeyRestoreCmd() (cli.Command, error) {
	return NewCommand(CommandArgs{
		Path:         []string{cst.NounEncryption, cst.NounDataKey + "/" + cst.Restore},
		RunFunc:      encryption{requests.NewHttpClient(), nil}.handleRestoreDataKey,
		SynopsisText: "Restore a previously soft-deleted data key",
		HelpText: fmt.Sprintf(`
Usage:
   • %[1]s %[2]s %[3]s --%[4]s mykeys/key1
		`, cst.NounEncryption, cst.NounDataKey, cst.Restore, cst.Path),
		FlagsPredictor: cli.PredictorWrappers{
			preds.LongFlag(cst.Path): cli.PredictorWrapper{preds.NewSecretPathPredictorDefault(), preds.NewFlagValue(preds.Params{Name: cst.Path, Shorthand: "r", Usage: fmt.Sprintf("Target %s to a %s (required)", cst.Path, cst.NounDataKey)}), false},
		},
		MinNumberArgs: 1,
...
func GetEncryptCmd() (cli.Command, error) {
	return NewCommand(CommandArgs{
		...
		SynopsisText: "Encrypt data using a data key",
		HelpText: fmt.Sprintf(`
Usage:
   ...
   • %[1]s %[2]s --%[3]s @mysecret.txt --%[4]s mykeys/key1
		...
		FlagsPredictor: cli.PredictorWrappers{
			preds.LongFlag(cst.Path):    cli.PredictorWrapper{preds.NewSecretPathPredictorDefault(), preds.NewFlagValue(preds.Params{Name: cst.Path, Shorthand: "r", Usage: fmt.Sprintf("Target %s to a %s (required)", cst.Path, cst.NounDataKey)}), false},
			preds.LongFlag(cst.Version): cli.PredictorWrapper{complete.PredictAnything, preds.NewFlagValue(preds.Params{Name: cst.Version, Shorthand: "", Usage: fmt.Sprintf("Version of the %s used for encryption/decryption", cst.NounDataKey)}), false},
...
func GetDecryptCmd() (cli.Command, error) {
	return NewCommand(CommandArgs{
		...
		SynopsisText: "Decrypt data using a data key that had performed encryption",
		HelpText: fmt.Sprintf(`
Usage:
   • %[1]s %[2]s --%[3]s 'hello world' --%[4]s mykeys/key1
   • %[1]s %[2]s --%[3]s @mysecret.txt"
		...
		FlagsPredictor: cli.PredictorWrappers{
			preds.LongFlag(cst.Path):    cli.PredictorWrapper{preds.NewSecretPathPredictorDefault(), preds.NewFlagValue(preds.Params{Name: cst.Path, Shorthand: "r", Usage: fmt.Sprintf("Target %s to a %s", cst.Path, cst.NounDataKey)}), false},
			preds.LongFlag(cst.Version): cli.PredictorWrapper{complete.PredictAnything, preds.NewFlagValue(preds.Params{Name: cst.Version, Shorthand: "", Usage: fmt.Sprintf("Version of the %s used for encryption/decryption", cst.NounDataKey)}), false},
...
func (e encryption) handleCreateDataKey(args []string) int {
	if e.outClient == nil {
...
func (e encryption) handleReadDataKey(args []string) int {
	if e.outClient == nil {
		e.outClient = format.NewDefaultOutClient()
	}
...
func (e encryption) handleDeleteDataKey(args []string) int {
	if e.outClient == nil {
		e.outClient = format.NewDefaultOutClient()
	}
...
func (e encryption) handleRestoreDataKey(args []string) int {
	if e.outClient == nil {
		e.outClient = format.NewDefaultOutClient()
	}
